...
class _OrderPageState extends State<OrderPage> {
  ...
  @override
  void initState() {
    context.read<ProductListBloc>().add(GetAllProducts(fromOrder: true,inventoryId: authentication.authenticatedUser.businessData?.businessId??0));
    super.initState();
  }
                                              ...
                                              onTap: () {
                                                ...
                                                if (productAddedListIds
                                                  ...
                                                  productAddedListIds.remove(
                                                      variantList[index].id);
                                                  productAddedList.removeWhere((product) {
                                                    return variantList.any((variant) => product.variantId == variantList[index].id);
                                                  });
                                                  productAddedId=0;
                                                  setState(() {});
                                                } else {
                                                  if(variantList[index].costingType=="dynamic price"){
                  ...
                  padding: EdgeInsets.only(
                    ...
                    child: StatefulBuilder(
                      builder: (context, setState) {
                        return AddDynamicProductCosting( onChanged:  (text) {
                     ...
                     onAdd: () {
                        ...
                        price=0;
                          ...
                          prodId: productId ?? 0,
                                                  ...
                                                  }else if(variantList[index].priceData?.sellingPrice==null && variantList[index].costingType!="dynamic price")
                                                    {
                                                      Fluttertoast.showToast(msg: "Oops! please add cost to this product");
                                                    }
                                                  else{
                                                    productAdded=null;
                                                    setState(() {
    ...
    AnimatedContainer(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.end,
        children: [
          ...
          },child: CircleAvatar(radius: 18,backgroundColor: Colors.white,child: Icon(Icons.close_rounded,color: ColorTheme.text,size: 18,),)),
          Spacer(),
          AnimatedContainer(
            duration: const Duration(milliseconds: 300),
            width: w,
              ...
              child: Row(
                ...
                children: [
                  ...
                  SizedBox(
                      ...
                      child: CommonButton(
                        onTap: () {
                          productAddedListIds.add(productAdded?.variantId ?? 0);
                          productAddedList.add(OrderLines(
                              image: productAdded?.image ?? '',
                              variantId: productAdded?.variantId ?? 0,
                              productId: productAdded?.productId ?? 1,
                              quantity: count,
                              sellingPrice: productAdded?.sellingPrice ?? 0,
                              variantName: productAdded?.variantName ?? "",
                              productName: productAdded?.productName ?? "",
                              deliveryNote: ""));
                          productConfirmedIds.add(productAdded?.variantId ?? 0);
                          productAdded = null;
                          count = 1;
                          setState(() {});
                        },
